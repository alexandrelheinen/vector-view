...
| ID | Requirement | Acceptance evidence |
| --- | --- | --- |
| M1 | Grasp motion is generated by the modern task-space controller. | `grasp_task_controller.py` invoked by `capture_comparison.sh`; snapshot `controller=grasp_task_controller`. |
| M2 | Capture occurs during the HOLD phase while both hands press the upper box. | Snapshot `phase=HOLD` and `both_on_box=true`. |
| M3 | Both hands maintain upper-box contact during HOLD at capture time. | Snapshot `both_on_box=true` and contact requirements V3/S1. |
